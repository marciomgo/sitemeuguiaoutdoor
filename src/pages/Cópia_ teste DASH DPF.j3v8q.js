...
$w.onReady(async function () {
    ...
    await verificarUltimaSubmeta(user.id); // 🔥 Garante que a submeta seja carregada ANTES de verificar progresso
...
function marcarDia(dataSelecionada) {
    ...
    wixData.query("familias")
        ...
        .then(() => {
         ...
         // ✅ Aqui está o gatilho centralizado:
            atualizarCalculoDistancia(); // 🔄 Gatilho para recalcular o resultado
        })
...
function calcularDistanciaDiaria() {
    ...
    // ✅ Recuperar distância total do banco antes de calcular novamente
    distanciaTotalTemp = 0; // Garante que a variável não fique indefinida

    wixData.query("distancias")
        .eq("userId", user.id)
        .find()
        .then((res) => {
            // 🔄 Corrigido: Agora atualiza a variável global corretamente
            distanciaTotalTemp = res.items.reduce((acc, item) => acc + (item.distancia || 0), 0);
            console.log("📌 Distância total recuperada do banco:", distanciaTotalTemp);

            // ✅ Aguarda os dados serem carregados antes de chamar atualizarCalculoDistancia
            setTimeout(() => {
                atualizarCalculoDistancia(); // 🔄 Centraliza o cálculo na função correta
            }, 300);
        })
    ...
    wixData.query("familias")
        ...
        .then((res) => {
            if (res.items.length > 0) {
                ...
                // ✅ Removido o cálculo de distanciaPorDia daqui
                // 🔄 Agora o cálculo é feito apenas em atualizarCalculoDistancia()
            }
        })
...
function atualizarCalculoDistancia() {
    ...
    if (typeof distanciaTotalTemp === "undefined" || isNaN(distanciaTotalTemp)) {
        console.warn("⚠️ distanciaTotalTemp ainda não carregada! Definindo como 0 temporariamente.");
        distanciaTotalTemp = 0; // ✅ Garante que não seja indefinida
    }

    let distanciaRestante = Math.max(metaDistancia - distanciaTotalTemp, 0);

    console.log(`📌 Após recarregar: Distância total já percorrida: ${distanciaTotalTemp} km`);
    console.log(`📌 Após recarregar: Distância restante correta: ${distanciaRestante} km`);
    console.log(`📌 Hoje é: ${hoje}`);
    console.log(`🔎 Dias futuros considerados no cálculo: ${diasFuturos}`);
    console.log(`🔎 Distância restante: ${distanciaRestante} km`);
    ...
    if (percentual >= 100) { 
        ...
        console.log("⚠️ Nenhum dia marcado. Solicitando definição no calendário.");
        $w("#resultadoCalculo").text = "Defina no calendário os dias para o desafio.";
    }
}
    ...
    wixData.query("familias")
        ...
        .then((res) => {
            if (res.items.length > 0) {
                ...
                // 🔹 Mantém os valores existentes ou define padrão caso não existam
                familiaExistente.ultimaSubmetaAtingida = familiaExistente.ultimaSubmetaAtingida || 0;  
                familiaExistente.metaAtingida = familiaExistente.metaAtingida || false;
                ...
                let novoRegistro = {
                    ...
                    ultimaSubmetaAtingida: 0, // 🔄 Começa em 0
                    metaAtingida: false // 🔄 Começa como "não atingida"
                };
    ...
    wixData.query("familias")
        ...
        .then((resFamilia) => {
            ...
            wixData.query("distancias")
                ...
                .find()
                .then((res) => {
                    console.log("📌 Dados retornados pelo banco:", res.items);

                    // ✅ Somente atualiza `distanciaTotalTemp` se houver distâncias válidas
                    let novaDistanciaTotal = res.items
                        .filter(item => item.distancia !== undefined)
                        .reduce((acc, item) => acc + (item.distancia || 0), 0);

                  if (novaDistanciaTotal > 0) {
    distanciaTotalTemp = novaDistanciaTotal; // ✅ Agora só atualiza se o valor for válido
}

console.log("📌 Distância total carregada ao recarregar a página:", distanciaTotalTemp);

// ✅ Aguarda um pequeno delay antes de recalcular a distância diária, garantindo que os dados foram carregados antes
setTimeout(() => {
    atualizarCalculoDistancia();
}, 500);
                    ...
                    console.log(`📌 Atualizando progresso: ${percentual}% (${distanciaTotalTemp} km)`);

                    $w("#progressBarWix").value = percentual;
                    $w("#totalDistancia").text = `${distanciaTotalTemp} km`;
                    ...
                    // ✅ Chama corretamente a função para exibir desafios
                    carregarDesafios(percentual);
                })
...
function verificarSubmeta(percentual) {
    ...
    // ✅ Verificar no banco qual foi a última submeta atingida para evitar repetição do popup
    wixData.query("familias")
        .eq("userId", wixUsers.currentUser.id)
        .find()
        .then((res) => {
            if (res.items.length === 0) {
                console.log("⚠️ Nenhum registro de família encontrado.");
                return;
            }

            let familia = res.items[0];
            let ultimaSubmetaSalva = familia.ultimaSubmetaAtingida || 0; // Se não existir, assume 0

            let submetasAtingidas = submetas.filter(s => s <= percentual && s > ultimaSubmetaSalva);
            ...
            // ✅ Salvar no banco de dados diretamente na coleção "distancias"
            let novoRegistro = {
            ...
            // ✅ Agora também atualiza a coleção "familias" para salvar a última submeta atingida
            familia.ultimaSubmetaAtingida = ultimaSubmetaAtivada; // Atualiza o campo corretamente

            wixData.update("familias", familia)
                .then(() => console.log("✅ Submeta salva corretamente no banco de familias: " + ultimaSubmetaAtivada))
                .catch(err => console.error("❌ Erro ao atualizar submeta na coleção familias:", err));

        })
        .catch(err => console.error("❌ Erro ao buscar família para verificar submetas atingidas:", err));
}
...
async function verificarUltimaSubmeta(userId) {
    try {
        let resultado = await wixData.query("familias")
            .eq("userId", userId)
            .find();
        ...
        if (resultado.items.length > 0) {
            ...
            // Recupera a última submeta atingida do banco de dados
            ultimaSubmetaAtivada = dados.ultimaSubmetaAtingida || 0;
            metaJaAtivada = dados.metaAtingida || false;

            // 🔥 CORREÇÃO: Atualizar submetasAtivadas com as submetas já atingidas
            let submetas = [25, 50, 75];
            submetas.forEach(s => {
                if (s <= ultimaSubmetaAtivada) {
                    submetasAtivadas[s] = true;
                }
            });

            console.log(`📌 Última submeta atingida carregada do banco: ${ultimaSubmetaAtivada}%`);
        }
    } catch (erro) {
...
function verificarMetaFinal(percentual) {
    ...
    .then((res) => {
        if (res.items.length > 0) {
            let familia = res.items[0];
            familia.metaAtingida = true;
            wixData.update("familias", familia)
                .then(() => console.log("✅ Meta final salva no banco."))
                .catch(err => console.error("❌ Erro ao atualizar meta final:", err));
        }
    });
